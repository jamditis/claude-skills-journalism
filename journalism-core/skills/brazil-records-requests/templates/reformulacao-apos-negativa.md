# Reformulação após negativa
*Refiling a denied request*

**Quando reformular em vez de recorrer:** negativas por pedido genérico
(art. 13, I) ou desproporcional (art. 13, II) do Decreto 7.724/2012 costumam
ser resolvidas mais rápido com um pedido novo e mais estreito do que com um
recurso. Um recurso pode levar meses; um pedido novo, 20 dias.

**Alcance:** essas hipóteses do Decreto 7.724/2012 aplicam-se ao Poder
Executivo federal. Para outro órgão, confirme a regra local antes de citar o
Decreto.

*When to refile rather than appeal: "generic" and "disproportionate" denials
usually resolve faster with a narrower new request — 20 days versus months.*

**Quando recorrer, e não reformular:** negativa por trabalho adicional
(art. 13, III), por dados pessoais ou por sigilo. Nesses casos a negativa pode
ser juridicamente frágil e vale contestá-la — inclusive porque o precedente
serve para os próximos pedidos. Para silêncio no Poder Executivo federal, use
uma reclamação, nos termos dos arts. 22 e 23 do Decreto 7.724/2012.

*When to appeal instead: "additional work", personal data, or classification.
Federal-executive silence uses the Decree complaint route.*

Se houver dúvida, faça as duas coisas em paralelo. São processos
independentes e não há custo.
*If unsure, do both in parallel. They're independent and free.*

---

Ao Serviço de Informação ao Cidadão (SIC) do(a) [ÓRGÃO]

Com fundamento na Lei nº 12.527/2011, apresento novo pedido de acesso à
informação, delimitado nos termos abaixo.

**Referência:** o pedido protocolado sob o nº [NÚMERO] foi respondido com a
alegação de [generalidade / desproporcionalidade]. O presente pedido reformula
o objeto para afastar essa alegação.

**Objeto delimitado:**

a) Documento ou base: [NOME EXATO DO SISTEMA, RELATÓRIO OU CONJUNTO]
b) Período: [DATA INICIAL] a [DATA FINAL]
c) Unidade administrativa: [SETOR, SE CONHECIDO]
d) Campos ou seções de interesse: [LISTAR]
e) Formato: [FORMATO DIGITAL JÁ DISPONÍVEL]

Caso o volume ainda seja considerado excessivo, requeiro o fornecimento
parcelado, começando pelo período de [RECORTE MENOR], nos termos do que o
próprio órgão indicar como exequível.

[NOME COMPLETO]
[CPF ou outro documento de identificação válido]
[DATA]

---

## Como estreitar / How to narrow

Reduza uma dimensão de cada vez, na seguinte ordem de eficácia:

1. **Tempo** — de cinco anos para um ano
2. **Unidade** — de todo o órgão para uma superintendência
3. **Campos** — de "todos os dados" para seis colunas
4. **Formato** — aceitar a exportação padrão do sistema, qualquer que seja

*Narrow one dimension at a time: time, then unit, then fields, then format.*

O oferecimento de fornecimento parcelado é importante: ele transfere ao órgão
o ônus de dizer o que é viável, e uma resposta que rejeita até o recorte menor
fica visivelmente frágil em eventual recurso.

*Offering phased delivery matters — it shifts the burden to the agency, and a
refusal even of the smallest slice looks weak on appeal.*
