...
|--------|-------------|----------|
| [autocontext](./autocontext/) | Cross-session knowledge persistence with skill evolution. Lessons accumulate per-skill, and `/autocontext:evolve` folds them back into skill files | `/autocontext:setup`, `/autocontext:remember`, `/autocontext:review`, `/autocontext:evolve` |
...
| [superjawn](./superjawn/) | Research-augmented fork of obra/superpowers. Default-on research phase fires before brainstorming, debugging, and writing-skills. v0.6.0 ships all 14 skills; v1.0.0 (disabling the upstream plugin) gates on at least 2 weeks of real use | invoked indirectly via skills (e.g. `superjawn:brainstorming`, `superjawn:debugging`, `superjawn:writing-plans`) |
